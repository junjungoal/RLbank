...
class BaseEnv(gym.Env):
    ...
    def _load_model(self, xml_path):
        ...
        self.action_space = spaces.Dict([
            ('default', spaces.Box(low=minima, high=maxima, dtype=np.float32))
        ])
    ...
    def _apply_external_force(self, verbose=False):
        xfrc = self.data.xfrc_applied
        force = (np.random.rand(6) * 2 - 1) * self._env_config['apply_force']
...
class EnvWrapper(gym.Wrapper):
    ...
    def _after_step(self, reward, terminal, info):
        ...
        if self._episode_length == self.max_episode_steps:
            self._terminal = True
            step_log['TimeLimit.truncated'] = not terminal

        if self._terminal:
            total_time = time.time() - self._episode_time
            step_log["episode_success"] = int(self._success)
            step_log["episode_reward"] = self._episode_reward
            step_log["episode_length"] = self._episode_length
            step_log["episode_time"] = total_time

        return self._terminal, step_log
    ...
    def observation(self, observation):
        return OrderedDict([('default', observation)])
